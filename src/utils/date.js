 ...
 */
export const getPreWeekDay = (pre = 7, now = new Date()) => {
...
/**
 * 获取指定时间的倒计时
 * @param {传入的时间戳 / 秒} timestamp
 * @param {指定天数} interval
 * @param {格式} format
 */
export const countDownTime = (timestamp, interval = 7, format = 'D天hh时mm分ss秒') => {
  const start = timestamp
  const duration = interval * 24 * 60 * 60
  ...
  if (diff > 0) {
    // 剩下天
    const lastDay = Math.floor(diff / 24 / 60 / 60)
    const lastTime = diff - lastDay * 24 * 60 * 60
    // 剩下时分秒
    const lastHour = Math.floor(lastTime / (60 * 60))
    const lastMinute = Math.floor(lastTime / 60 % 60)
    const lastSecond = lastTime % 60
    const obj = {
      D: lastDay,
      h: lastHour,
      m: lastMinute,
      ...
      DD: padTime(lastDay),
      hh: padTime(lastHour),
      mm: padTime(lastMinute),
      ss: padTime(lastSecond)
    }
    return format.replace(/(DD?|hh?|mm?|ss?)/g, f => obj[f])
  } else {
    return diff
  }
}
